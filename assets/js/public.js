...
function renderUmkmCards() {
  ...
  grid.innerHTML = list.map(u => `
    <div class="card card-umkm" data-open="${esc(u.id)}" role="button" tabindex="0">
      ${u.foto_url ? `<img class="thumb contain" style="object-fit:contain !important; background:var(--cream);" src="${esc(imgUrl(u.foto_url))}" alt="${esc(u.nama)}" />` : `<div class="thumb thumb-ph">🛍️</div>`}
      <div class="body">
  ...
  grid.querySelectorAll("[data-open]").forEach(card => {
    const open = (id) => {
      ...
      openModal(`
        ...
        ${u.foto_url ? `<img class="modal-cover contain" style="object-fit:contain !important; background:var(--cream);" src="${esc(imgUrl(u.foto_url))}" alt="${esc(u.nama)}" />` : `<div class="modal-cover ph">🛍️</div>`}
...
function setupRealtime() {
  console.log("Supabase Realtime: Menghubungkan...");
  sb.channel('public-changes')
    .on('postgres_changes', { event: '*', schema: 'public' }, payload => {
      console.log("Realtime event diterima dari tabel:", payload.table, payload);
